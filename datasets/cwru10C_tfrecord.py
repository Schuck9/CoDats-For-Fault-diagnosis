...
def cwru_load(data_catogories,hp):
        data_name = data_catogories.split("_")[1]
        _cwru= CWRU(data_name, hp, 384)
...
def subdataset_split(original_dataset =None, feature = "Radius"):
        output_dir = os.path.join("datasets", "new_cwru")
...
def save_dataset(dataset_name, output_dir,hp, seed=0):
    
    """ Save single dataset """
    train_filename = os.path.join(output_dir,
        tfrecord_filename(dataset_name+"_"+hp, "train"))
    valid_filename = os.path.join(output_dir,
        tfrecord_filename(dataset_name+"_"+hp, "valid"))
    test_filename = os.path.join(output_dir,
        tfrecord_filename(dataset_name+"_"+hp, "test"))
    ...
    train_data,train_labels,valid_data,valid_labels,test_data,test_labels = cwru_load(dataset_name,hp)
...
def main():
    main_dataset = ["cwru10C"]
    Transfer_dataset = ["12DriveEndFault","12FanEndFault","48DriveEndFault"]
    output_dir = os.path.join("datasets", "new_cwru")
    hp_list = ['1797','1772','1750','1730']
    adaptation_problems = []
    ...
    for dataset_name in adaptation_problems:
        for hp in hp_list:
            save_dataset(dataset_name, output_dir,hp)
