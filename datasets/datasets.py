...
class cwru_data(Dataset):
    ...
    def __init__(self, users,*args, **kwargs):
        ...
        super().__init__(cwru_data.num_classes, cwru_data.class_labels,
            None, None, cwru_data.feature_names,*args, **kwargs)

    def load(self):
        self._cwru= CWRU("12DriveEndFault", '1797', 384)
        self._cwru.X_train = np.array(self._cwru.X_train,dtype=np.float32)
        self._cwru.X_test = np.array(self._cwru.X_test,dtype=np.float32)
        X_train_shape = self._cwru.X_train.shape
        X_test_shape = self._cwru.X_test.shape
        self._cwru.X_train = np.reshape(self._cwru.X_train,(X_train_shape[0],X_train_shape[1],1))
        self._cwru.X_test = np.reshape(self._cwru.X_test,(X_test_shape[0],X_test_shape[1],1))
...
class jnu_data(Dataset):
    users = [
            "0","1","2",
            "0_700","0_600","0_500","0_400","0_300","0_200","0_100",
            "1_700","1_600","1_500","1_400","1_300","1_200","1_100",
            "2_700","2_600","2_500","2_400","2_300","2_200","2_100",
        ]
    num_classes = 4
    already_normalized = True
    feature_names = [
